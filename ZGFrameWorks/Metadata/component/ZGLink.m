                    ...
                    inContainer: (ZGContainer*) container
{
  ...
  if (selectedImage)
  {
    [uiButton setBackgroundImage: selectedImage
  ...
  if (fontColor)
  {
    ...
    [uiButton setTitleColor: [UIColor blackColor]
                   forState: UIControlStateNormal];
  }
                            ...
                            WithRect: (CGRect) drawRect
{
  ...
  LOG(@"uiButton.frame : %.f, %.f, %.f, %.f", uiButton.frame.origin.x, uiButton.frame.origin.y, uiButton.frame.size.width, uiButton.frame.size.height);
  return drawRect;
}
                  ...
                  AttributeVale: (NSString*) value
{
  ...
  LOG(@"uiButton.frame.y : %.f", uiButton.frame.origin.y);
  if ([attributeName isEqualToString:MAR_TAG_ATTR_BACK_IMAGE]) {
    ...
    if (backColor) {
      ...
      [uiButton setBackgroundColor: backColor];
    }
  }
    ...
    if ([self isInContainerClass: [ZGMenu class]])
    {
      font = [UIFont boldSystemFontOfSize: fSize];
    } else {
      font = [UIFont systemFontOfSize: fSize];
    }
    [uiButton.titleLabel setFont: font];
  }
